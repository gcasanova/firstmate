...
export function createContextMeter(thresholds = CONTEXT_THRESHOLDS, { notifiedTokens = [] } = {}) {
  const knownThresholds = new Set(thresholds.map((threshold) => threshold.tokens));
  const notified = new Set(
    Array.isArray(notifiedTokens) ? notifiedTokens.filter((tokens) => knownThresholds.has(tokens)) : [],
  );
  ...
  return {
    ...
    snapshot() {
      return { notifiedTokens: [...notified].sort((left, right) => left - right) };
    },
  };
}
