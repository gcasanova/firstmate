...
2. Commit the change and run the relevant checks.
3. Push the branch to your fork and open a pull request targeting `main`.

GitHub Actions runs the repository's deterministic lint, test, and invariant checks for every pull request.
...
Tracked changes to firstmate itself - `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/` - ship through a feature-branch PR and require an explicit merge approval.
...
When a task selects the no-mistakes delivery mode, its worker follows the installed no-mistakes version's SKILL.md and live `axi` help instead of duplicating gate mechanics in firstmate docs.
...
When a no-mistakes run publishes evidence, never hand-commit `.no-mistakes/` paths onto a feature branch; CI rejects them as tracked personal fleet paths.
