...
Only that state line may reclassify an orphaned green CI monitor as held-for-merge done or a run record the `daemon status` probe leaves unverified as unknown, never the raw run record.
