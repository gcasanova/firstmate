...
test_no_mistakes_is_not_a_bootstrap_dependency() {
  local case_dir fakebin out
  case_dir="$TMP_ROOT/no-mistakes-optional"
  mkdir -p "$case_dir/home/config"
  ...
  [ -z "$out" ] || fail "bootstrap treated absent no-mistakes as required: $out"
  pass "bootstrap does not probe or require no-mistakes"
}
